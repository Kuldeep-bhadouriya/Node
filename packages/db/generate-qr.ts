...
async function generateQRCodes(outputDir: string) {
  ...
  for (const [branch, branchStudents] of branchMap) {
    const branchCode = branchStudents[0].cardNo.split('/')[2] || branch.match(/\(([^)]+)\)/)?.[1] || branch.replace(/[^a-z0-9]/gi, '').toUpperCase();
    ...
    for (const student of branchStudents) {
      try {
        ...
        const profileUrl = baseUrl + '/student/' + student.id + '?token=' + encodeURIComponent(qrToken);
        ...
        const cardNumber = student.cardNo.split('/').at(-1) || student.cardNo.replace(/[^a-z0-9]/gi, '_');
